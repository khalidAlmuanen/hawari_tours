    ...
    () => [
      ...
      },
      {
        ...
        mega: [
          ...
          { href: '/gallery', title: locale === 'ar' ? 'المعرض' : 'Gallery', sub: locale === 'ar' ? 'صور ولقطات' : 'Photo Gallery', icon: '📸' },
          { href: '/hotels', title: locale === 'ar' ? 'حجز الفنادق' : 'Hotels', sub: locale === 'ar' ? 'إقامة فاخرة' : 'Luxury stays', icon: '🏨' },
          { href: '/cars', title: locale === 'ar' ? 'تأجير السيارات' : 'Cars', sub: locale === 'ar' ? 'أسطول فاخر' : 'Luxury fleet', icon: '🚘' },
      ...
      },
      {
        ...
        icon: '✍️',
        desc: locale === 'ar' ? 'مقالات وقصص' : 'Articles & stories',
      },
      {
        href: '/news',
      ...
      },
      {
        ...
        mega: [
          ...
          { href: '/testimonials', title: locale === 'ar' ? 'آراء العملاء' : 'Testimonials', sub: locale === 'ar' ? 'قصص المسافرين' : 'Traveler Stories', icon: '💬' },
          { href: '/reports', title: locale === 'ar' ? 'التقارير' : 'Reports', sub: locale === 'ar' ? 'دراسات وأبحاث' : 'Studies & Research', icon: '📊' },
    ...
    <>
      ...
      <header className="fixed top-0 left-0 right-0 z-50">
        ...
        <div className={`mx-auto max-w-7xl px-4 transition-all duration-500 ${isScrolled ? 'pt-3' : 'pt-4'}`}>
          <div
            className={`relative rounded-3xl border shadow-2xl transition-all duration-500 ${isScrolled
          ...
          >
            ...
            <div className="relative h-20 flex items-center justify-between px-4 sm:px-6">
              ...
              <Link href="/" className="flex items-center gap-3 group">
                <div className="relative rounded-[1.6rem] bg-white/95 px-3 py-2 shadow-xl ring-1 ring-black/5 transition-all duration-300 group-hover:scale-[1.03] dark:bg-white/10 dark:ring-white/10">
                  <div className="absolute inset-0 rounded-[1.6rem] opacity-0 blur-xl transition-opacity group-hover:opacity-80 bg-gradient-to-br from-orange-500/30 via-amber-400/20 to-emerald-400/20" />
                  <BrandLogo
                    alt="Hawari Tours"
                    priority
                    className="relative w-[62px] sm:w-[72px]"
                  />
                </div>
                ...
                {nav.map((item) => {
                  ...
                  return (
                    ...
                    >
                      <Link
                        href={item.href}
                        className={`relative px-4 py-2.5 rounded-2xl font-bold text-sm transition-all duration-300 flex items-center gap-2 ${item.cta
              ...
              <div className="flex items-center gap-2">
                ...
                {user ? (
                  <div className="hidden lg:block relative" ref={userMenuRef}>
                    <button
                      ...
                      className={`group flex items-center gap-3 px-3 py-2 rounded-2xl border transition-all ${isScrolled
                  ...
                  <Link
                    href="/login"
                    className={`hidden lg:flex items-center gap-2 px-4 py-2.5 rounded-2xl font-black transition-all ${isScrolled
                ...
                <button
                  ...
                  className={`hidden sm:flex items-center gap-2 px-3 py-2 rounded-2xl font-black transition-all ${isScrolled ? 'bg-black/5 dark:bg-white/5 text-gray-800 dark:text-gray-100' : 'bg-white/10 text-white'
                ...
                <button
                  ...
                  className={`px-3 py-2 rounded-2xl transition-all border border-white/10 ${isScrolled ? 'bg-black/5 dark:bg-white/5' : 'bg-white/10'
                    } hover:shadow-xl`}
                  title="Theme"
                >
                ...
                <button
                  ...
                  className={`lg:hidden px-3 py-2 rounded-2xl border border-white/10 transition-all ${isScrolled ? 'bg-black/5 dark:bg-white/5' : 'bg-white/10'
                    } hover:shadow-xl`}
      ...
      {/* Mobile side drawer */}
      <aside
        className={`lg:hidden fixed top-0 right-0 z-50 h-[100dvh] w-[86%] max-w-sm transition-transform duration-300 ${drawerOpen ? 'translate-x-0' : 'translate-x-full'
      ...
      >
        ...
        {/* ✅ SCROLL AREA */}
        <div
          className="p-4 space-y-2 overflow-y-auto overscroll-contain flex-1 min-h-0 touch-pan-y"
            ...
            if (!hasMega) {
              return (
                <Link
                  ...
                  className={`group flex items-center gap-3 p-4 rounded-3xl border transition-all ${isActive(item.href)
