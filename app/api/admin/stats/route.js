...
export async function GET(request) {
  ...
  try {
    ...
    const [
      ...
      prisma.booking.findMany({
        ...
        include: {
          ...
          hotel: { select: { name: true, nameAr: true } },
          car: { select: { name: true, nameAr: true } }
        }
      }),
    ...
    const formattedBookings = latestBookings.map((booking) => ({
      id: booking.id,
      tour: booking.tour?.title || booking.hotel?.name || booking.car?.name || 'Unknown',
      tourAr: booking.tour?.titleAr || booking.hotel?.nameAr || booking.car?.nameAr || 'غير معروف',
