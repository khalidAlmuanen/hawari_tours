...
export default function VisaTab() {
    ...
    try {
      ...
      if (result.success) {
        if (Array.isArray(result.data)) {
          ...
          setMeta({
            sectionTitleAr: result.data?.sectionTitleAr || '',
            sectionTitleEn: result.data?.sectionTitleEn || '',
            headlineAr: result.data?.headlineAr || '',
            headlineEn: result.data?.headlineEn || '',
            headlineHighlightAr: result.data?.headlineHighlightAr || '',
            headlineHighlightEn: result.data?.headlineHighlightEn || '',
            requirementsTitleAr: result.data?.requirementsTitleAr || '',
            requirementsTitleEn: result.data?.requirementsTitleEn || ''
          })
        }
      }
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      ...
      <form onSubmit={saveMeta} className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-lg border border-gray-200 dark:border-gray-700">
        <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-6">
          {isAr ? 'عناوين قسم التأشيرات' : 'Visa Section Titles'}
        </h3>
        <div className="grid md:grid-cols-2 gap-4">
          <input
            className="w-full px-4 py-3 border rounded-xl dark:bg-gray-700 dark:border-gray-600"
            placeholder="Badge Title (En)"
            value={meta.sectionTitleEn}
            onChange={(e) => setMeta({ ...meta, sectionTitleEn: e.target.value })}
          />
          <input
            ...
            value={meta.sectionTitleAr}
            onChange={(e) => setMeta({ ...meta, sectionTitleAr: e.target.value })}
            dir="rtl"
          />
          <input
            className="w-full px-4 py-3 border rounded-xl dark:bg-gray-700 dark:border-gray-600"
            ...
            onChange={(e) => setMeta({ ...meta, headlineEn: e.target.value })}
          />
          <input
            className="w-full px-4 py-3 border rounded-xl text-right dark:bg-gray-700 dark:border-gray-600"
            placeholder="العنوان الرئيسي (عربي)"
            value={meta.headlineAr}
            onChange={(e) => setMeta({ ...meta, headlineAr: e.target.value })}
            dir="rtl"
          />
          <input
            className="w-full px-4 py-3 border rounded-xl dark:bg-gray-700 dark:border-gray-600"
            placeholder="Headline Highlight (En)"
            value={meta.headlineHighlightEn}
            onChange={(e) => setMeta({ ...meta, headlineHighlightEn: e.target.value })}
          />
          <input
            className="w-full px-4 py-3 border rounded-xl text-right dark:bg-gray-700 dark:border-gray-600"
            placeholder="تمييز العنوان (عربي)"
            value={meta.headlineHighlightAr}
            onChange={(e) => setMeta({ ...meta, headlineHighlightAr: e.target.value })}
            dir="rtl"
          />
          <input
            className="w-full px-4 py-3 border rounded-xl dark:bg-gray-700 dark:border-gray-600"
            placeholder="Requirements Title (En)"
            value={meta.requirementsTitleEn}
            onChange={(e) => setMeta({ ...meta, requirementsTitleEn: e.target.value })}
          />
          <input
            className="w-full px-4 py-3 border rounded-xl text-right dark:bg-gray-700 dark:border-gray-600"
            placeholder="عنوان المتطلبات (عربي)"
            value={meta.requirementsTitleAr}
            onChange={(e) => setMeta({ ...meta, requirementsTitleAr: e.target.value })}
            dir="rtl"
          />
        </div>
        <div className="mt-6 flex justify-end">
          <button type="submit" className="px-6 py-3 bg-blue-600 text-white rounded-xl font-bold hover:bg-blue-700">
            {isAr ? 'حفظ العناوين' : 'Save Titles'}
          </button>
        </div>
      </form>
