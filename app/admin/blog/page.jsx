    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            const [blogsRes, authorsRes, tagsRes] = await Promise.all([
                fetch('/api/admin/blog', { cache: 'no-store', credentials: 'include' }),
                fetch('/api/admin/blog/authors', { cache: 'no-store', credentials: 'include' }),
                fetch('/api/admin/blog/tags', { cache: 'no-store', credentials: 'include' })
            ])
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const res = await fetch(url, {
                ...
                credentials: 'include',  // ✅ ضروري لإرسال الـ Cookie مع الطلب
            ...
            if (result.success) {
                ...
                // عرض رسالة الخطأ التفصيلية من الـ server
                const errMsg = result.details
                    ? `${result.error}: ${result.details}`
                    : (result.error || 'Operation failed')
                showError(errMsg)
                console.error('Blog save error:', result)
            }
        } catch (error) {
            console.error('Blog submit fetch error:', error)
