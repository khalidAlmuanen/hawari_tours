...
// ✨ Features: Dynamic Data, Better design, animations, map integration
// ═══════════════════════════════════════════════════════════════════════
...
export default function ContactPage() {
  ...
  // Fallback Data
  const phones = contactInfo?.contact?.phones || { primary: '+967 772 371 581', whatsapp: '+967 772 371 581' }
  const emails = contactInfo?.contact?.emails || { info: 'info@hawari.tours' }
  const address = contactInfo?.contact?.addresses?.[0] || {
    addressEn: 'Hadiboh, Socotra, Yemen',
    addressAr: 'حديبو، سقطرى، اليمن',
    lat: '12.65', lng: '54.02'
  }
  ...
  const companyName = contactInfo?.company?.[isAr ? 'nameAr' : 'nameEn'] || (isAr ? 'حوار تورز' : 'Hawari Tours')
  const heroTitle = contactInfo?.company?.[isAr ? 'heroTitleAr' : 'heroTitleEn'] || (isAr ? 'تواصل معنا' : 'Get in Touch')
  const heroSubtitle = contactInfo?.company?.[isAr ? 'heroSubtitleAr' : 'heroSubtitleEn'] || (isAr
  ...
  const addressTitleAr = address.titleAr || 'قرب المطار'
  const addressTitleEn = address.titleEn || 'Near Airport'
  const weekdaysDays = isAr
    ? (workingHours.weekdays?.daysAr || workingHours.weekdays?.days || 'الأحد - الخميس')
    : (workingHours.weekdays?.daysEn || workingHours.weekdays?.days || 'Sunday - Thursday')
  const weekendDays = isAr
    ? (workingHours.weekend?.daysAr || workingHours.weekend?.days || 'السبت')
    : (workingHours.weekend?.daysEn || workingHours.weekend?.days || 'Saturday')
  const closedDays = isAr
    ? (workingHours.closedAr || workingHours.closed || 'الجمعة')
    : (workingHours.closedEn || workingHours.closed || 'Friday')
  const mapSrc = address.mapUrl?.trim()
    ? address.mapUrl.trim()
    : `https://www.google.com/maps?q=${encodeURIComponent(address.lat || '12.65')},${encodeURIComponent(address.lng || '54.02')}&z=14&output=embed`
  ...
  const contactMethods = [
    {
      ...
      link: `tel:${phones.primary.replace(/\s+/g, '')}`,
    ...
    },
    {
      ...
      link: `https://wa.me/${phones.whatsapp.replace(/\D/g, '')}`,
    ...
    },
    {
      ...
      value: { ar: address.addressAr, en: address.addressEn },
      link: '#map',
      gradient: 'from-orange-500 to-red-600',
      description: { ar: addressTitleAr, en: addressTitleEn }
    }
  ]
  ...
  // Social Media
  const socialMediaList = contactInfo?.socialMedia || {}
  const facebook = socialMediaList.facebook || { url: '#', active: false }
  const instagram = socialMediaList.instagram || { url: '#', active: false }
  const twitter = socialMediaList.twitter || { url: '#', active: false }
  const youtube = socialMediaList.youtube || { url: '#', active: false }
  const tiktok = socialMediaList.tiktok || { url: '#', active: false }
  const linkedin = socialMediaList.linkedin || { url: '#', active: false }
  ...
  const socialMedia = [
    ...
    tiktok.active && {
      name: 'TikTok',
      icon: (<svg className="w-6 h-6" viewBox="0 0 24 24" fill="currentColor"><path d="M16.5 2h-3v12.2a3.7 3.7 0 11-3-3.62V7.54a6.7 6.7 0 102.37 12.98A6.7 6.7 0 0016.5 14V8.7a8.5 8.5 0 004.5 1.3V7a5.5 5.5 0 01-4.5-5z" /></svg>),
      link: tiktok.url,
      color: 'hover:text-gray-900 dark:hover:text-white'
    },
    linkedin.active && {
      name: 'LinkedIn',
      icon: (<svg className="w-6 h-6" viewBox="0 0 24 24" fill="currentColor"><path d="M20.447 20.452H17.2v-5.569c0-1.329-.027-3.039-1.852-3.039-1.853 0-2.136 1.445-2.136 2.939v5.669H9.064V9h3.112v1.561h.044c.434-.82 1.494-1.684 3.073-1.684 3.287 0 3.893 2.164 3.893 4.977v6.598zM5.337 7.433a1.804 1.804 0 110-3.608 1.804 1.804 0 010 3.608zM6.956 20.452H3.72V9h3.235v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.727v20.545C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.273V1.727C24 .774 23.2 0 22.222 0h.003z" /></svg>),
      link: linkedin.url,
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors">
          ...
          <div className="grid lg:grid-cols-5 gap-12">
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  <div className="grid md:grid-cols-2 gap-6">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        required
                        className={`w-full px-4 py-3 border-2 rounded-xl bg-white dark:bg-gray-900 text-gray-900 dark:text-white transition-all outline-none ${focusedField === 'name'
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onBlur={() => setFocusedField(null)}
                        required
                        className={`w-full px-4 py-3 border-2 rounded-xl bg-white dark:bg-gray-900 text-gray-900 dark:text-white transition-all outline-none ${focusedField === 'email'
                            ? 'border-green-500 ring-4 ring-green-200 dark:ring-green-900/30'
                            : 'border-gray-300 dark:border-gray-600'
                          }`}
                  ...
                  <div className="grid md:grid-cols-2 gap-6">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onBlur={() => setFocusedField(null)}
                        required
                        className={`w-full px-4 py-3 border-2 rounded-xl bg-white dark:bg-gray-900 text-gray-900 dark:text-white transition-all outline-none ${focusedField === 'phone'
                            ? 'border-green-500 ring-4 ring-green-200 dark:ring-green-900/30'
                            : 'border-gray-300 dark:border-gray-600'
                          }`}
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      required
                      rows={6}
                      className={`w-full px-4 py-3 border-2 rounded-xl bg-white dark:bg-gray-900 text-gray-900 dark:text-white transition-all outline-none resize-none ${focusedField === 'message'
                          ? 'border-green-500 ring-4 ring-green-200 dark:ring-green-900/30'
                          : 'border-gray-300 dark:border-gray-600'
                        }`}
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-6 border border-gray-200 dark:border-gray-700">
                ...
                <div className="space-y-3 text-gray-600 dark:text-gray-400">
                  <div className="flex justify-between">
                    <span>{weekdaysDays}</span>
                    <span className="font-semibold">{isAr ? workingHours.weekdays?.openAr : workingHours.weekdays?.openEn}</span>
                  </div>
                  {workingHours.weekend && (
                    <div className="flex justify-between">
                      <span>{weekendDays}</span>
                      <span className="font-semibold">{isAr ? workingHours.weekend?.openAr : workingHours.weekend?.openEn}</span>
                    </div>
                  )}
                  <div className="flex justify-between">
              ...
              <div className="bg-gradient-to-br from-green-500 to-emerald-600 rounded-2xl shadow-lg p-6 text-white">
                ...
                <a
                  href={`https://wa.me/${phones.whatsapp.replace(/\D/g, '')}`}
                  target="_blank"
                  rel="noopener noreferrer"
      ...
      <section id="map" className="py-16 bg-white dark:bg-gray-800">
        <div className="container mx-auto px-4 max-w-7xl">
          ...
          <div className="relative h-[400px] bg-gray-200 dark:bg-gray-700 rounded-2xl overflow-hidden shadow-xl border border-gray-300 dark:border-gray-600">
            {/* Map Integration */}
            <iframe
              src={mapSrc}
              width="100%"
            ...
            <div className="absolute bottom-4 left-4 bg-white dark:bg-gray-900 p-4 rounded-xl shadow-lg max-w-xs">
